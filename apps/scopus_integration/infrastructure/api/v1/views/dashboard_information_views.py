...
class DashboardInformationViewSet(viewsets.ViewSet):
    ...
    def get_system_health(self, request):
        try:
            ...
            mongo_topics = CountryTopics.objects.filter(topic_name__ne=' ').filter(topic_name__ne='').count()
            latest_acumulated = CountryAcumulated.objects.order_by('-year').first()
            mongo_articles = latest_acumulated.total_articles if latest_acumulated else 0
