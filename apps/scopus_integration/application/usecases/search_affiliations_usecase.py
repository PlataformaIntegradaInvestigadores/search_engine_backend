...
class Search:
    ...
    def __init__(
        self, url=None, query=None, facets=None, view=None, field=None, searchType=None
    ):
        self.num_res = None
    ...
    def execute(self, client: ScopusClient = None, get_all=False):
        try:
            ...
            existing_articles = {article.scopus_id for article in Article.nodes.all()}
            if get_all is True:
                while self.num_res < self.tot_num_res:
                    ...
                    for article_ in api_response["search-results"]["entry"]:
                        try:
                            ...
                            doi = (
                                article_.get("prism:doi", "")
                                if article_.get("prism:doi", "")
                                else None
                            )
                        except ValueError as e:
                            print("Error on article validation: ", e)
                            logger.log(
                                logging.ERROR, f"Error on article validation: {e}"
                            )
                            continue
                        ...
                        if scopus_id in existing_articles:
                            logger.log(
                                logging.INFO,
                                f"Article with Scopus ID {scopus_id} already exists.",
                            )
                            ...
                            logger.log(
                                logging.INFO,
                                f"Creating article with Scopus ID {scopus_id}",
                            )
                            Article.from_json(article_, client)

                        existing_articles.add(
                            scopus_id
                        )  # Update the set with the new article's Scopus I
