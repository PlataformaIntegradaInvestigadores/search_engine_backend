...
class PopulateView(APIView):
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(
                description="Datalake populated successfully",
                examples={"application/json": {"message": "datalake populated"}},
            ),
            500: OpenApiResponse(
    ...
    def post(self, request):
        try:
            ...
            return Response(
                {"message": "Analytics DB populated"}, status=status.HTTP_200_OK
            )
